...
var CreateCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		for _, createWorkspaceDto := range createWorkspaceDtos {
			ws, err := apiclient_util.GetWorkspace(createWorkspaceDto.Id, true)
			if err != nil {
				return err
			}
		...
		return workspace_common.OpenIDE(chosenIdeId, activeProfile, createWorkspaceDtos[0].Name, *createdWorkspaces[0].Info.ProviderMetadata, YesFlag, gpgKey)
	},
}
